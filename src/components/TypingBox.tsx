...
export default function TypingBox() {
  const askAI = useAITeacher(
    (state: { askAI: (question: string) => void }) => state.askAI
  );
  const loading = useAITeacher((state: { loading: boolean }) => state.loading);
  ...
  return (
    <div className='z-10 max-w-[600px] flex space-y-6 flex-col bg-gradient-to-tr  from-slate-300/30 via-gray-400/30 to-slate-600-400/30 p-4  backdrop-blur-md rounded-xl border-slate-100/30 border'>
      <div>
        <h2 className='text-white font-bold text-xl'>How to say in Korean?</h2>
        <p className='text-white/65'>
          Type a sentence you want to say in Korean and AI teacher will
      ...
      {loading ? (
        ...
        <div className='gap-3 flex'>
          <input
            ...
            placeholder='Have you ever been to Korea?'
            value={question}
