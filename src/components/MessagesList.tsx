...
export const MessagesList = () => {
  const messages = useAITeacher((state: { messages: any[] }) => state.messages);
  //   const playMessage = useAITeacher((state: { playMessage: (message: any) => void }) => state.playMessage);
  const { currentMessage } = useAITeacher();
  const english = useAITeacher((state: { english: boolean }) => state.english);
  const hangeul = useAITeacher((state: { hangeul: boolean }) => state.hangeul);
  const classroom = useAITeacher(
    (state: { classroom: string }) => state.classroom
  );
  ...
  const renderKorean = (korean: { word: string; reading?: string }[]) => (
        ...
        <div key={i}>
          <div className='flex'>
            <div className='flex-grow'>
              ...
              {renderKorean(message.answer.korean)}
            </div>
            {currentMessage === message ? (
              <button
                className='text-white/65'
                onClick={() => {
                  /* Implement stopMessage function */
                }}
              >
                <svg
